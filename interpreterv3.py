...
class Interpreter(InterpreterBase):
    ...
    def instantiate(self, class_name, line_num_of_statement):
        if class_name in self.class_index:
            ...
            if class_name_list[0] in self.tclass_index:
                tclass_def = self.tclass_index[class_name_list[0]]
                class_def = tclass_def.create_class(class_name_list[1:])
            else:
                super().error(
    ...
    def is_a_subtype(self, suspected_supertype, suspected_subtype):
        return self.type_manager.is_a_subtype(
            suspected_supertype, suspected_subtype
        )
    ...
    def check_type_compatibility(self, typea, typeb, for_assignment=False):
        return self.type_manager.check_type_compatibility(
            typea, typeb, for_assignment
        )
...
class VariableDef:
    ...
    def __init__(self, var_type, var_name, value=None):
        ...
        self.name = var_name
        self.value = (
            value if value is not None else create_default_value(var_type)
        )
...
class ClassDef:
    def __init__(self, class_source, interpreter, map_param_types=None):
        self.interpreter = interpreter
        self.name = class_source[1]
        self.class_source = class_source
        self.map_param_types = map_param_types
    ...
    def __check_method_names_and_types(self, method_def):
        return_type = method_def.return_type.type_name
        if self.map_param_types and return_type in self.map_param_types:
            return_type = self.map_param_types[return_type]
...
class ObjectDef:
    ...
    def __init__(
        self, interpreter, class_def, anchor_object=None, trace_output=False
    ):
    ...
    def call_method(
        self, method_name, actual_params, super_only, line_num_of_caller
    ):
        # check to see if we have a method in this class or its base class(es) matching this signature
        if (
            self.__get_obj_with_method(self, method_name, actual_params)
            is None
        ):
            self.interpreter.error(
                ErrorType.NAME_ERROR,
    ...
    def __set_variable_aux(self, env, var_name, value, line_num):
        ...
        if self.__set_local_or_param(
            ...
            return
        if self.__set_field(
            var_name, value, line_num
        ):  # may report a type error
            return
        self.interpreter.error(
            ErrorType.NAME_ERROR,
            "unknown field/variable " + var_name,
            line_num,
        )
    ...
    def __create_map_of_operations_to_lambdas(self):
        ...
        self.binary_ops[InterpreterBase.INT_DEF] = {
            ...
            "==": lambda a, b: Value(
                ObjectDef.BOOL_TYPE_CONST, a.value() == b.value()
            ),
            "!=": lambda a, b: Value(
                ObjectDef.BOOL_TYPE_CONST, a.value() != b.value()
            ),
        ...
        self.binary_ops[InterpreterBase.STRING_DEF] = {
            "+": lambda a, b: Value(
                ObjectDef.STRING_TYPE_CONST, a.value() + b.value()
            ),
            "==": lambda a, b: Value(
                ObjectDef.BOOL_TYPE_CONST, a.value() == b.value()
            ),
        ...
        self.binary_ops[InterpreterBase.BOOL_DEF] = {
            "&": lambda a, b: Value(
                ObjectDef.BOOL_TYPE_CONST, a.value() and b.value()
            ),
            "|": lambda a, b: Value(
                ObjectDef.BOOL_TYPE_CONST, a.value() or b.value()
            ),
            "==": lambda a, b: Value(
                ObjectDef.BOOL_TYPE_CONST, a.value() == b.value()
            ),
...
class TypeManager:
    ...
    def is_a_subtype(self, suspected_supertype, suspected_subtype):
        if not self.is_valid_type(
            suspected_supertype
        ) or not self.is_valid_type(suspected_subtype):
            return False
    ...
    def check_type_compatibility(self, typea, typeb, for_assignment):
        ...
        if not template and self.is_a_subtype(
